...
deepspeed --include localhost:0,1,2,3 --master_port 29501 tinyllava/train/train.py \
    ...
    --pretrained_model_path output/Final/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-pretrain-share \
    --output_dir output/Final/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune-share \
    --num_train_epochs 1 \
