...
deepspeed --include localhost:0,1,2,3 --master_port 29501 tinyllava/train/custom_finetune.py \
    ...
    --pretrained_model_path output/Final/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune-share \
    --output_dir output/Final/tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune-down-share-e6 \
    --num_train_epochs 6 \
    ...
    --run_name tiny-llava-${LLM_VARIANT}-${VT_VARIANT}-${VERSION}-finetune_down
